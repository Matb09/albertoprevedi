...
function initGallery() {
    ...
    const classicScrollContainer = document.getElementById('gallery-scroll');
    const galleryRoots = document.querySelectorAll('[data-gallery-root]');

    if (classicScrollContainer) {
        initClassicGallery(images, classicScrollContainer);
    }
    ...
    galleryRoots.forEach((root) => {
        ...
        root.querySelectorAll('[data-gallery-track]').forEach((track) => {
            const categoryKey = track.getAttribute('data-gallery-track') || 'gare';
            const categoryImages = categoryKey === 'all'
                ? images
                : images.filter((image) => image.category === categoryKey);
            renderGalleryTrack(track, categoryImages, categoryKey);

            const sectionNode = track.closest('.gallery-category-section');
            if (sectionNode) {
                initGalleryNav(sectionNode, track);
            }
        });
    });
}

function initClassicGallery(images, scrollContainer) {
    ...
    const mountGallery = () => {
        ...
        renderGalleryTrack(scrollContainer, images, 'all');
        initClassicGalleryNav();
    };
...
function renderGalleryTrack(container, images, categoryKey) {
    container.innerHTML = '';

    if (!images.length) {
        showGalleryEmpty(container, categoryKey);
        return;
    }

    const categoryLabel = (GALLERY_CATEGORY_META[categoryKey] && GALLERY_CATEGORY_META[categoryKey].label) || 'Gallery';

    images.forEach((imageData, index) => {
        const item = document.createElement('div');
        item.className = 'gallery-item';
        item.innerHTML = `
      <img alt="Alberto Prevedi - ${categoryLabel} - Foto ${index + 1}" loading="lazy" decoding="async" width="560" height="760">
...
function showGalleryEmpty(container, categoryKey = '') {
    const categoryLabel = (GALLERY_CATEGORY_META[categoryKey] && GALLERY_CATEGORY_META[categoryKey].label) || 'questa sezione';
    container.innerHTML = `
    <div class="gallery-empty" style="width:100%;">
      <p>Le foto per ${categoryLabel.toLowerCase()} saranno disponibili a breve.</p>
    </div>
  `;
}

/* Gallery carousel navigation */
function initGalleryNav(scopeNode, scrollContainer) {
    const prevBtn = scopeNode.querySelector('[data-gallery-prev]');
    const nextBtn = scopeNode.querySelector('[data-gallery-next]');
...
function initClassicGalleryNav() {
